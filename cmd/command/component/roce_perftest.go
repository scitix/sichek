...
func NewRoCEPerftestCmd() *cobra.Command {
	...
	ibPerftestCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if err != nil {
				logrus.WithField("perftest", "roce").Errorf("failed to get the verbose: %v", err)
				passed = false
			}
			if !verbose {
			...
			if passed {
				res, err := perftest.CheckRoCEPerfHealth(testType, expectedBandwidthGbps, expectedLatencyUs, ibDevice, netDevice, size, duration, gid, qpNum, useGDR, rdmaCM, verbose)
				if err != nil {
					logrus.WithField("perftest", "roce").Error(err)
				}
